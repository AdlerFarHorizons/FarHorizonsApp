...
function parse_trackfile(data) {
    ...
    // Seperate each line into an aray
...
function plot_track2() {
	...
	document.getElementById('BalloonStats').innerHTML="<b>Current Dynamics:</b><br> "+this.track[N-1][4]+" ft, "+this.track[N-1][3]+" mph, "+this.track[N-1][2]+"&deg; heading";
...
{   
	...
	for (i=chart1.series[this.series].data.length;i<this.track.length;i++) {
	    ...
	    {chart2.series[this.series].addPoint([eval(this.track[i][3]),
						  eval(this.track[i][4])],false,false);}
        //{chart3.series[this.series].addPoint([eval(this.track[i][3]),
		//				  eval(this.track[i][4])],false,false);}
	}            
	
	// now redraw sidebar graphs
	{ chart1.redraw();chart2.redraw();}//chart3.redraw();}
    }
...
function addMarker(markerLayer,lineE) {
    ...
    if (markerLayer.id=="kc9lig") {
            var icon=iconpurple.clone(); 
    }

    if (markerLayer.id=="kc9lhw") {
        if (altitude<maxaltitude) {
            var icon=icongreen.clone(); //green if lower than max altitude
        } else {
            var icon=iconblue.clone(); //blue if greater than max altitude
            maxaltitude=altitude;
        }
    }
    console.log( "KC9LHWmaxaltitude:", maxaltitude);
    
    if (markerLayer.id=="wb9sky") {
            var icon=iconblue.clone(); 
    }
