class ChaseVehiclesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @chase_vehicle.save
        format.html { redirect_to @chase_vehicle, notice: 'Chase vehicle was successfully created.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to chase_vehicles_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def chase_vehicle_params
      params.require(:chase_vehicle).permit(:description, :identifier, :chase_server_id)
    end
end
